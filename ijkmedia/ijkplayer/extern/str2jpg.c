...
/**
 * \fn      fm_ini_stream2jpg
 * \brief   initial the idr2jpg
 * \param   
 * \return  
 */
void fm_ini_stream2jpg(int flg)
{
	if(!flg){
		return;
	}	
...
/**
 * \fn      fm_unini_stream2jpg
 * \brief   remove the idr2jpg
 * \param   
 * \return  
 */

void fm_unini_stream2jpg(void)
{
...
int scale_img(AVCodecContext *pCodecCtx,AVFrame *src_picture,AVFrame *dst_picture,int nDstW,int nDstH)
{
    ...
    fm_dbg_log(NULL, AV_LOG_INFO,"aizj enter %s,line=%d",__FUNCTION__,__LINE__);
...
int  yuv2jpg(AVFrame *in_pFrame,int out_width,int out_high,const char* out_pFile) 
{
	...
	fm_dbg_log(NULL, AV_LOG_INFO,"aizj enter %s,line=%d,%s",__FUNCTION__,__LINE__,out_pFile);
	...
	//Output URL
	fm_dbg_log(NULL, AV_LOG_INFO,"aizj Line=%d", __LINE__);
	...
	if (!pCodec){
		av_log(NULL, AV_LOG_ERROR,"aizj codec not found,%s,line=%d",__FUNCTION__,__LINE__);
		iRet = -1;
		goto fail;
	}
	if (avcodec_open2(pCodecCtx, pCodec,NULL) < 0){
		av_log(NULL, AV_LOG_ERROR,"aizj codec not open,%s,line=%d",__FUNCTION__,__LINE__);
		iRet = -1;
		goto fail;
	}
	...
	if (!picture_buf)
	{
		av_log(NULL, AV_LOG_ERROR,"aizj %s,line=%d",__FUNCTION__,__LINE__);
		iRet = -1;
		goto fail;
	}
	...
	fm_dbg_log(NULL, AV_LOG_INFO,"aizj %s,line=%d",__FUNCTION__,__LINE__);
 ...
 * \return   0,  succeed;
            <0,  failed;
 */
int fm_stream2jpg(const char *in_streamName,const char *out_jpgName) 
{
  AVFormatContext *pFormatCtx = NULL;
  int             i, videoStream;
  AVCodecContext  *pCodecCtx = NULL;
  ...
  fm_dbg_log(NULL, AV_LOG_INFO,"Cw=%d,Ch=%d,%s,line=%d",pCodecCtx->width,pCodecCtx->height,__FUNCTION__,__LINE__);
  ...
  avpicture_fill((AVPicture *)pFrameRGB, buffer,PIX_FMT_YUV420P,FM_JPG_WIDTH,FM_JPG_HIGH);
  
  // Read frames and save first five frames to disk
  while(av_read_frame(pFormatCtx, &packet) >=0) {
  	
	  // Is this a packet from the video stream?
	  fm_dbg_log(NULL, AV_LOG_INFO,"si=%d,vs=%d,%s,line=%d",packet.stream_index,videoStream,__FUNCTION__,__LINE__);
	  if(packet.stream_index == videoStream) 
	  	{

		  // Decode video frame
	      avcodec_decode_video2(pCodecCtx, pFrame, &frameFinished,&packet);
		  fm_dbg_log(NULL, AV_LOG_INFO,"pFrame=%d,%s,line=%d",pFrame,__FUNCTION__,__LINE__);
